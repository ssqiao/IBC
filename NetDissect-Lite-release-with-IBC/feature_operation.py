...
class FeatureOperator:
    def __init__(self):
        ...
        self.mean = [103.939, 116.779, 123.68]  # BGR qss

    def feature_extraction(self, model=None, memmap=True):
        loader = self.loader
        # extract the max value activaiton for each image
        maxfeatures = [None] * len(settings.FEATURE_NAMES)  # for each img, the max value of each channel
        wholefeatures = [None] * len(settings.FEATURE_NAMES)
        features_size = [None] * len(settings.FEATURE_NAMES)  # total num features, img_num * channels * h * w
        ...
        if memmap:
            ...
            for i, (mmap_file, mmap_max_file) in enumerate(zip(mmap_files, mmap_max_files)):
                if os.path.exists(mmap_file) and os.path.exists(mmap_max_file) and features_size[i] is not None:
                    ...
                    print('features file missing, loading from scratch')
                    skip = False
            if skip:
        ...
        for batch_idx, batch in enumerate(loader.tensor_batches(bgr_mean=self.mean)):
            ...
            input = torch.from_numpy(input[:, ::-1, :, :].copy())  # BRG to RGB
            input.div_(255.0 * 0.224)  # to tensor & div std
            if settings.GPU:
                input = input.cuda()
            input_var = V(input, volatile=True)
            logit = model.forward(input_var)  # TODO qss
            # logit = model.forward(input_var, masked=False)
            while np.isnan(logit.data.cpu().max()):
                print("nan")  # which I have no idea why it will happen
                del features_blobs[:]
                logit = model.forward(input_var)
            features_blobs.append(logit.data.cpu().numpy())
            ...
            for concept_map in batch:
                ...
                for unit_id in range(units):
                    ...
                    if feature_map.max() > threshold[unit_id]:
                        ...
                        # mask = upsample.upsampleL(fieldmap, feature_map, shape=(concept_map['sh'], concept_map['sw']),
                        # reduction=reduction)
