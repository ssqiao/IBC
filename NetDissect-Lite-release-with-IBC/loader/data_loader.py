...
class SegmentationData(AbstractSegmentation):
    ...
    def __init__(self, directory, categories=None, require_all=False):
        ...
        # qss read the index.csv, category.csv and label.csv into self variables
        ...
        # Filter out images with insufficient data if require all fields
        filter_fn = partial(
        ...
        self.category_map = {}  # number as index -> code
        self.category_unmap = {}  # code as index --> number
        self.category_label = {}  # raw c_*.csv info
        ...
        # number as index --> category type [0-5], 0 'color', 1 'object' ...
    ...
    # static func, get the segmented img or class label for  categories ['color', 'object', ...], ignore others
    @classmethod
    def resolve_segmentation(cls, m, categories=None):
        ...
        directory, row = m
        result = {}
        for cat, d in row.items():  # row is one dict contain one row info in index.csv
...
# used for segmentation data, read label.csv
...
# used for segmentation data, read index.csv
...
# used for segmentation data, just has one field, true
...
# used for segmentation data, must have all fields, true
...
# used for FeatureOperation, use thread to load imgs
class SegmentationPrefetcher:
    ...
    def __init__(self, segmentation, split=None, randomize=False,
        ...
        categories: a list of categories to include in each batch. raw or segmented imgs
        batch_size: number of data items for each batch.
        ahead: the number of batches to prefetch ahead.
        '''
    ...
    # one by one return the raw broden img info, without any img read op.
    def next_job(self):
        if self.index < 0:
            return None
    ...
    def refill_tasks(self):
        ...
        while len(self.result_queue) < self.ahead:
            ...
            if len(data) == 0:
                return
            self.result_queue.append(self.pool.map_async(prefetch_worker, data))  # read the raw or segmented imgs

    # close the prefetcher thread for loading data
    def close(self):
...
# used for segmentation prefetch, one by one process the img
def prefetch_worker(d):
    if d is None:
        return None
    j, typ, m, fn, categories, segmentation_shape = d  # idx, Class Name, img full info, file name, target cat, seg size
    ...
    if categories is None or 'image' in categories:
        segs['image'] = imread(fn)   # read the raw img have not been segmented
    return segs


# used for prefetch_worker, scale the loaded seg img
...
# used for segmentation prefetch, init the thread for load data
def setup_sigint():
    import threading
...
# used for segmentation prefetch
